...
Также храним эфемерный контекст меню (ephemeral_message_id), чтобы кнопки внутри
эфемерного меню редактировали его спец-методом, а не обычным edit.

Хранилище — в памяти (dict). При рестарте очищается: старые меню перестанут
проверяться (можно пересоздать /start).
"""
import time

# (chat_id, message_id) -> (owner_id, ts)
_owners: dict[tuple[int, int], tuple[int, float]] = {}
# режим эфемерного меню на пользователя: uid -> bool (хочет эфемерное меню в группе)
_eph_pref: dict[int, bool] = {}
_MAX = 5000            # максимум записей в памяти
_TTL = 24 * 3600       # запись живёт сутки


def wants_ephemeral(uid: int) -> bool:
    """Предпочтение пользователя: показывать меню эфемерно в группе."""
    return _eph_pref.get(uid, False)


def set_ephemeral(uid: int, on: bool) -> None:
    _eph_pref[uid] = on
