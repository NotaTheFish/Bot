...
# буквы столбцов для КНОПОК и координатных подписей (латиница, совпадает с
# эмодзи-буквами 🇦-🇯 в шапке поля, чтобы игрок не путался)
COLS = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"]

# эмодзи-подписи (одинаковой ширины с клетками) — и в поле, и на кнопках.
# Игрок может заменить их на премиум через слоты/свободные замены.
CORNER_EMO = "🔳"
COL_EMO = ["🔲", "🟥", "🟧", "🟨", "🟩", "🟦", "🟪", "⬛", "⬜", "🟫"]
ROW_EMO = ["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"]
...
def _render_grid(cell_fn) -> str:
    """Сетка 10x10. Подписи — эмодзи той же ширины, что клетки, чтобы колонки
    совпадали в любом клиенте Telegram. Столбцы А-К, строки 1-10 эмодзи-цифрами."""
    # эмодзи-буквы столбцов (широкие, совпадают по ширине с клетками)
    col_labels = COL_EMO
    row_labels = ROW_EMO
    header = CORNER_EMO + "".join(col_labels)   # угол + столбцы
    lines = [header]
    for r in range(SIZE):
        row = "".join(cell_fn(r, c) for c in range(SIZE))
        lines.append(f"{row_labels[r]}{row}")
    return "\n".join(lines)
