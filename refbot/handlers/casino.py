...
async def cb_case_open(c: CallbackQuery):
    ...
    await _cnt.casino_event(uid, "case", won, is_jackpot=(mult >= 10), bet=cost, currency=cur, mult=mult)
...
async def _spin_wheel(target, uid: int, bet_cur: int, cur: str, edit: bool):
    ...
    from services import counters as _cnt
    await _cnt.casino_event(uid, "wheel", won, is_jackpot=(mult >= 10), bet=bet_cur, currency=cur, mult=mult)
...
async def cb_mines_open(c: CallbackQuery, state: FSMContext):
    ...
    if field[idx] == 0:
        ...
        from services import counters as _cnt
        await _cnt.casino_event(uid, "mines", 0, bet=g["bet_cur"], currency=g["cur"], mult=0.0)  # проигрыш
...
async def _mines_cashout(c, state, forced: bool):
    ...
    from services import counters as _cnt
    await _cnt.casino_event(uid, "mines", won, is_jackpot=(mult >= 10), bet=g["bet_cur"], currency=cur, mult=mult)
...
async def cb_case_open5(c: CallbackQuery):
    ...
    for mult, won in results:
        ...
        await _cnt.casino_event(uid, "case", won, is_jackpot=(mult >= 10), bet=one_cost, currency=cur, mult=mult)
