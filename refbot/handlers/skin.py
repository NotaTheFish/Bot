...
async def cb_chats(c: CallbackQuery):
    if await deny_view(c):
        return
    chats = await db.chats_overview()
    if not chats:
        return await c.answer("Нет чатов. /шайнуть — рефералка, /шимм — рулетка, "
                              "/шимшайнуть — конкурс.", show_alert=True)
    ...
    for ch in chats:
        ...
        svc_line = " · ".join(svc) if svc else "ничего не активно"
        lines.append(f"{mark} <b>{title}</b>\n   {svc_line}")

    await ui.edit(c.message,
        "📢 <b>Чаты</b>\n\n"
        + "\n\n".join(lines) +
        "\n\n<i>Рефералка — /шайнуть · Рулетка — /шимм · Конкурс — /шимшайнуть</i>"
        + ("\n\nНажми на чат, чтобы управлять рефералкой." if manage
           else "\n\n<i>Только просмотр.</i>"),
...
async def cb_sk_wait(c: CallbackQuery, state: FSMContext):
    ...
    await ui.edit(c.message,
        ...
        f"<i>⚠️ Чтобы премиум-реакция заработала, в НАСТРОЙКАХ ЧАТА "
        f"(Управление чатом → Реакции) выбери режим <b>«Только выбранные»</b> "
        f"и добавь В СПИСОК именно этот эмодзи. Режима «Все реакции» "
        f"НЕДОСТАТОЧНО — Telegram разрешает боту кастомную реакцию только через "
        f"явный список. Не добавлен — бот ставит запасную 👀. "
        f"Обычные «часы» ⌛ как реакцию Telegram не принимает вовсе. "
        f"Это ограничение Telegram.</i>",
